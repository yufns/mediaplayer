...
public class GSYADVideoPlayer extends StandardGSYVideoPlayer {
    ...
    @Override public GSYVideoViewBridge getGSYVideoManager() {
        GSYVideoADManager
            .instance()
            .initContext(getContext().getApplicationContext());
        return GSYVideoADManager.instance();
    }

    @Override protected boolean backFromFull(Context context) {
        return GSYVideoADManager.backFromWindowFull(context);
    }

    @Override protected void releaseVideos() {
        GSYVideoADManager.releaseAllVideos();
    }

    @Override protected int getFullId() {
        return GSYVideoADManager.FULLSCREEN_ID;
    }

    @Override protected int getSmallId() {
    ...
    protected void changeAdUIState() {
        ...
        if (mBottomContainer != null) {
            int color = (isFirstPrepared) ? Color.TRANSPARENT : getContext()
                .getResources()
