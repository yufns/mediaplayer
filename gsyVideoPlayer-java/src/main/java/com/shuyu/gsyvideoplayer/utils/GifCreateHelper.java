...
public class GifCreateHelper {
    ...
    public GifCreateHelper(StandardGSYVideoPlayer standardGSYVideoPlayer,
        GSYVideoGifSaveListener gsyVideoGifSaveListener) {
     ...
     */
    public GifCreateHelper(StandardGSYVideoPlayer standardGSYVideoPlayer,
        GSYVideoGifSaveListener gsyVideoGifSaveListener, int delay, int inSampleSize, int scaleSize,
        int frequencyCount) {
        ...
        new Thread(new Runnable() {
            @Override public void run() {
                if (mPicList.size() > 2) {
                    ...
                    createGif(path, mPicList, mDelay, mSampleSize, mScaleSize,
    ...
    private void startSaveBitmap() {
        ...
        mPlayer.saveFrame(file, new GSYVideoShotSaveListener() {
            @Override public void result(boolean success, final File file) {
                mSaveShotBitmapSuccess = true;
                if (success) {
