...
public class GSYVideoHelper {
    ...
    public GSYVideoHelper(Context context, StandardGSYVideoPlayer player) {
        ...
        this.mWindowViewContainer = (ViewGroup) (CommonUtil.scanForActivity(context)).findViewById(
    ...
    private void resolveToFull() {
        mSystemUiVisibility = ((Activity) mContext)
            .getWindow()
            .getDecorView()
            .getSystemUiVisibility();
        CommonUtil.hideSupportActionBar(mContext, mVideoOptionBuilder.isHideActionBar(),
            mVideoOptionBuilder.isHideStatusBar());
        ...
        mGsyVideoPlayer
            .getFullscreenButton()
            .setImageResource(mGsyVideoPlayer.getShrinkImageRes());
        mGsyVideoPlayer
            .getBackButton()
    ...
    private void resolveMaterialAnimation() {
        ...
        saveLocationStatus(mContext, mVideoOptionBuilder.isHideActionBar(),
            mVideoOptionBuilder.isHideStatusBar());
        FrameLayout.LayoutParams lpParent =
            new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT);
        FrameLayout frameLayout = new FrameLayout(mContext);
        frameLayout.setBackgroundColor(Color.BLACK);
        FrameLayout.LayoutParams lp =
            new FrameLayout.LayoutParams(mNormalItemSize[0], mNormalItemSize[1]);
    ...
    private void resolveToNormal() {
        ...
        mHandler.postDelayed(new Runnable() {
            @Override public void run() {
                ...
                mGsyVideoPlayer
                    .getFullscreenButton()
                    .setImageResource(mGsyVideoPlayer.getEnlargeImageRes());
                mGsyVideoPlayer
                    .getBackButton()
                ...
                if (mVideoOptionBuilder.getVideoAllCallBack() != null) {
                    ...
                    mVideoOptionBuilder
                        .getVideoAllCallBack()
                        .onQuitFullscreen(mVideoOptionBuilder.getUrl(),
                            mVideoOptionBuilder.getVideoTitle(), mGsyVideoPlayer);
                }
                if (mVideoOptionBuilder.isHideKey()) {
                    showNavKey(mContext, mSystemUiVisibility);
                }
                CommonUtil.showSupportActionBar(mContext, mVideoOptionBuilder.isHideActionBar(),
                    mVideoOptionBuilder.isHideStatusBar());
            }
        }, delay);
    }
    ...
    private void resolveMaterialToNormal(final GSYVideoPlayer gsyVideoPlayer) {
        if (mVideoOptionBuilder.isShowFullAnimation()
            && mFullViewContainer instanceof FrameLayout) {
            int delay = mOrientationUtils.backToProtVideo();
            mHandler.postDelayed(new Runnable() {
                @Override public void run() {
                    TransitionManager.beginDelayedTransition(mFullViewContainer);
                    FrameLayout.LayoutParams lp =
                        (FrameLayout.LayoutParams) gsyVideoPlayer.getLayoutParams();
                    lp.setMargins(mNormalItemRect[0], mNormalItemRect[1], 0, 0);
    ...
    private void resolveChangeFirstLogic(int time) {
        ...
        if (mVideoOptionBuilder.getVideoAllCallBack() != null) {
            ...
            mVideoOptionBuilder
                .getVideoAllCallBack()
                .onEnterFullscreen(mVideoOptionBuilder.getUrl(),
                    mVideoOptionBuilder.getVideoTitle(), mGsyVideoPlayer);
        }
    }
    ...
    private boolean removeWindowContainer() {
        if (mWindowViewContainer != null
            && mWindowViewContainer.indexOfChild(mGsyVideoPlayer) != -1) {
     ...
     * @param playBtn 播放按键
     */
    public void addVideoPlayer(final int position, View imgView, String tag, ViewGroup container,
        View playBtn) {
    ...
    public boolean backFromFull() {
        ...
        if (mFullViewContainer != null && mFullViewContainer.getChildCount() > 0) {
            isFull = true;
            resolveMaterialToNormal(mGsyVideoPlayer);
        } else if (mWindowViewContainer != null
            && mWindowViewContainer.indexOfChild(mGsyVideoPlayer) != -1) {
            isFull = true;
            resolveMaterialToNormal(mGsyVideoPlayer);
        }
    ...
    public void releaseVideoPlayer() {
        ...
        mPlayPosition = -1;
        TAG = "NULL";
        if (mOrientationUtils != null) mOrientationUtils.releaseListener();
    }
