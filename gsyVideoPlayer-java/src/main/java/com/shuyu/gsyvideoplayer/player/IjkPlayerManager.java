...
public class IjkPlayerManager implements IPlayerManager {
    ...
    @Override public void initVideoPlayer(Context context, Message msg,
        ...
        mediaPlayer =
            (ijkLibLoader == null) ? new IjkMediaPlayer() : new IjkMediaPlayer(ijkLibLoader);
        mediaPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
        mediaPlayer.setOnNativeInvokeListener(new IjkMediaPlayer.OnNativeInvokeListener() {
            @Override public boolean onNativeInvoke(int i, Bundle bundle) {
                return true;
            }
        });
    ...
    @Override public void showDisplay(Message msg) {
    ...
    @Override public void setNeedMute(boolean needMute) {
        if (mediaPlayer != null) {
            if (needMute) {
    ...
    @Override public void setSpeedPlaying(float speed, boolean soundTouch) {
        if (mediaPlayer != null) {
            mediaPlayer.setSpeed(speed);
            mediaPlayer.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "soundtouch",
                (soundTouch) ? 1 : 0);
        }
    }
