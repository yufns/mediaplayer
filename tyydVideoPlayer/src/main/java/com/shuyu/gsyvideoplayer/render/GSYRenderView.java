...
public class GSYRenderView {
    ...
    public int getWidth() {
        return (mShowView != null) ? mShowView
            .getRenderView()
            .getWidth() : 0;
    }
    ...
    public View getShowView() {
        if (mShowView != null) return mShowView.getRenderView();
        return null;
    }
     ...
     * 添加播放的view
     */
    public void addView(final Context context, final ViewGroup textureViewContainer,
        final int rotate, final IGSYSurfaceListener gsySurfaceListener,
        ...
        if (GSYVideoType.getRenderType() == GSYVideoType.SUFRACE) {
            mShowView = GSYSurfaceView.addSurfaceView(context, textureViewContainer, rotate,
                gsySurfaceListener, videoParamsListener);
        } else if (GSYVideoType.getRenderType() == GSYVideoType.GLSURFACE) {
            mShowView =
                GSYVideoGLView.addGLView(context, textureViewContainer, rotate, gsySurfaceListener,
                    videoParamsListener, effect, transform, customRender, mode);
        } else {
            mShowView = GSYTextureView.addTextureView(context, textureViewContainer, rotate,
                gsySurfaceListener, videoParamsListener);
        }
    }
    ...
    public Bitmap initCover() {
        if (mShowView != null) return mShowView.initCover();
        return null;
    }
    ...
    public Bitmap initCoverHigh() {
        if (mShowView != null) return mShowView.initCoverHigh();
        return null;
    }
    ...
    public void taskShotPic(GSYVideoShotListener gsyVideoShotListener, boolean shotHigh) {
        if (mShowView != null) mShowView.taskShotPic(gsyVideoShotListener, shotHigh);
    }
    ...
    public void saveFrame(final File file, final boolean high,
        ...
        if (mShowView != null) mShowView.saveFrame(file, high, gsyVideoShotSaveListener);
    }
    ...
    public static void addToParent(ViewGroup textureViewContainer, View render) {
        ...
        if (textureViewContainer instanceof RelativeLayout) {
            RelativeLayout.LayoutParams layoutParams =
                new RelativeLayout.LayoutParams(params, params);
    ...
    public static int getTextureParams() {
        ...
        return (typeChanged) ? ViewGroup.LayoutParams.WRAP_CONTENT
