...
public class GSYVideoADManager extends GSYVideoBaseManager {
    ...
    @SuppressLint("StaticFieldLeak") private static GSYVideoADManager videoManager;
     ...
     * @return 返回是否全屏
     */
    @SuppressWarnings("ResourceType") public static boolean backFromWindowFull(Context context) {
        boolean backFrom = false;
        ViewGroup vp = (ViewGroup) (CommonUtil.scanForActivity(context)).findViewById(
            Window.ID_ANDROID_CONTENT);
        View oldF = vp.findViewById(FULLSCREEN_ID);
        if (oldF != null) {
            backFrom = true;
            hideNavKey(context);
            if (GSYVideoADManager
                .instance()
                .lastListener() != null) {
                GSYVideoADManager
                    .instance()
     ...
     */
    @SuppressWarnings("ResourceType") public static boolean isFullState(Activity activity) {
        ViewGroup vp = (ViewGroup) (CommonUtil.scanForActivity(activity)).findViewById(
            Window.ID_ANDROID_CONTENT);
