 ...
 */
@SuppressLint("ViewConstructor") public class GSYVideoGLViewSimpleRender
    ...
    @Override synchronized public void onFrameAvailable(SurfaceTexture surface) {
        mUpdateSurface = true;
    }
     ...
     * 设置滤镜效果
     */
    @Override public void setEffect(GSYVideoGLView.ShaderInterface shaderEffect) {
    ...
    @Override public GSYVideoGLView.ShaderInterface getEffect() {
        return mEffect;
    }
    ...
    protected void takeBitmap(GL10 glUnused) {
        if (mTakeShotPic) {
            ...
            if (mGSYVideoShotListener != null) {
                Bitmap bitmap = createBitmapFromGLSurface(0, 0, mSurfaceView.getWidth(),
                    mSurfaceView.getHeight(), glUnused);
